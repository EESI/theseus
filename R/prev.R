...
#'
#' Function 'prev' (prevalence plot) tabulates the prevalance and abundance of each taxa in a phyloseq object and plots the results as a ggplot object. This may assist the user in determining what filtering and preprocessing steps should be taken regarding the removal of low count taxa. 
#'
#' @param PS (required) A phyloseq object.
#' @param taxon Taxonomic level to be displayed. Defaults to 'Phylum'. 
#' @param n_taxa The number of taxa to display at the given 'taxon' level. The most abundant taxa at that group are selected. Defaults to 10. 
#' @param abund_theshold User selected value for the lowest acceptable total abundance. Defaults to 3. 
...
#'
#' @references
#' Callahan BJ, Sankaran K, Fukuyama JA et al. Bioconductor Workflow for Microbiome Data Analysis: from raw reads to community analyses. F1000Research 2016, 5:1492 (doi: 10.12688/f1000research.8986.2)  
#' Perraudeau F, Risso D, Street K et al. Bioconductor workflow for single-cell RNA sequencing: Normalization, dimensionality reduction, clustering, and lineage inference. F1000Research 2017, 6:1158 (doi:10.12688/f1000research.12122.1)
#' 
#' @examples
#' \dontrun{
#' library(theseus)
