...
#' @param idx Indexes (within f_path and r_path) identifying specific .fastq files to be used for analysis
#' @param percentile The percentile to be targeted . Defaults to .25 (i.e. the first quartile).
#' @param amp_length Intra-primer amplicon length. Calculated destance in base-pairs between primers. Used to determine region of no overlap. Both 'amp_length' and 'min_overlap' must be provided for these calculations. 
#' @param min_overlap The minimum amount of overlap between the two reads. Used to determine region of no overlap. Both 'amp_length' and 'min_overlap' must be provided for these calculations. 
...
#' Users can (re)run 'qualcontour' with different 'percentile' values to visualize how the quality scores varies in shape. \code{\link[dada2]{plotQualityProfile}} in the 'dada2' package provides an elegant way of looking at the quality profiles for the forward or reverse reads. 
#'
