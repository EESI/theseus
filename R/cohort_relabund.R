...
#'
#' This function plots the relative abundance of taxa within a phyloseq object 'PS' according to thier pre-deterimined cohort memberships. See Details for more information. 
#'
#' @param PS (required) A phyloseq object.
#' @param xvar (required) Variable in sample_data(PS) to be displayed on the x-axis. Defaults to 'SampleID'.
#' @param taxfill Taxonomic level to display in plot. Defaults to 'Phylum'. 
#' @param comp1 (required) First comparison (hence 'comp1') object of 'DESeqResults' class, or a dataframe with similar structure. In the case that a 'DESeqResults' class object is not being used, the object must contain a 'log2FoldChange' vector/column. Row names must be a subset (but not necessarily a proper subset) of taxa_names(PS). See Details for more information. 
#' @param comp2 (required) Second comparision (hence 'comp2') object. Refer to documentation for 'comp1' for remaining details. 
#' @param comp1lab Labels for comparision 1. Defaults to c('Decreased Comp1','No Change Comp1','Increased Comp1').
#' @param comp2lab Labels for comparision 2. Defaults to c('Decreased Comp2','No Change Comp2','Increased Comp2'). 
#' @param justdata Return only the data table (no plot). Defaults to FALSE. 
#' @param PSisRelAbund Does the PS object contain compositional (relative abundance) taxa counts? Defaults to FALSE. 
#' 
#' @details
#' \subsection{General}{
#' The results from a single pairwise comparision, such as pre- and post-treatment, carried out with the DESeq2 package can be plotted or read and interpreted in tabular form with relative ease. When two pairwise comparisions are being performed, interpreting the results becomes more difficult. This function is intended to assist with interpreting the results from multiple differential abundance analyses carried out with the DESeq2 r-package. This function takes a phyloseq object ('PS') and two 'DESeqResults' objects ('comp1', 'comp2') and plots the relative abundance of taxa within 'PS', partitioning the taxa according to thier membership to one the 9 possible cohort combinations determined by their values specified within 'comp1' and 'comp2'.  
#' }
#' \subsection{Approach}{
#' The \code{\link[DESeq2]{DESeq}} function carries out differential abundance testing and produces a 'DESeqDataSet' object. The \code{\link[DESeq2]{results}} function can be used to access the results and create a 'DESeqResults' object, which is a subclass of DataFrame. Note that the 'alpha' parameter for \code{\link[DESeq2]{results}} can be used to specify the significance level of the test being performed. NOTE: Testing with DESeq2 must be carried out, and the non-significant taxa should be removed from the 'comp1' and 'comp2' objects before using this function. Using the log2FoldChange columns in 'comp1' and 'comp2' this function identifies which taxa decrease, do not change, or increase over course of both comparisions. Because there are three options for both comparisions there are 3^2=9 possible combinations, or cohorts, which an OTU may fall into. These cohort assignments are used when plotting the relative abundance plot. 
#' }
#' 
#'
#' @return A ggplot object.
#'
#' @seealso \code{\link[phyloseq]{plot_bar}} \code{\link[phyloseq]{transform_sample_counts}} \code{\link[DESeq2]{DESeq}} \code{\link[DESeq2]{results}} \code{\link[DESeq2]{DESeqDataSet}}
#' 
#' @examples
#' \dontrun{
#' cohort_relabund(
